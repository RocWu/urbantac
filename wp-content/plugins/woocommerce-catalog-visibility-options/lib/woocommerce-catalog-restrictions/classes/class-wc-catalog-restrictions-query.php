            ...
            if ($wc_catalog_restrictions->get_setting('_wc_restrictions_locations_enabled', 'yes') == 'yes') {
                ...
                if (true || $filtered === false) {
                    ...
                    if ($allowed_by_location) {
                        $exclusion_sql = " SELECT ID FROM $wpdb->posts WHERE post_type='product' AND ID NOT IN ('" . implode("','", $allowed_by_location) . "')";
                ...
                if ($filtered === false) {
                    ...
                    if ($allowed_by_role) {
                        $exclusion_sql = " SELECT ID FROM $wpdb->posts WHERE post_type='product' AND ID NOT IN ('" . implode("','", $allowed_by_role) . "')";
                        $filtered = $wpdb->get_col($exclusion_sql);
                    }
