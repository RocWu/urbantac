...
class WC_Catalog_Restrictions_Location_Picker_Widget extends WP_Widget {
    ...
    function form($instance) {
        ...
        <p><label for="<?php echo $this->get_field_id('location_not_set_title'); ?>"><?php _e('Location not set title:', 'woocommerce') ?></label>
            <input type="text" class="widefat" id="<?php echo esc_attr($this->get_field_id('location_not_set_title')); ?>" name="<?php echo esc_attr($this->get_field_name('location_not_set_title')); ?>" value="<?php if (isset($instance['location_not_set_title'])) echo esc_attr($instance['location_not_set_title']); else echo __('Choose your location', 'wc_catalog_restrictions'); ?>" /></p>

        <p><label for="<?php echo $this->get_field_id('location_set_title'); ?>"><?php _e('Logged set title:', 'woocommerce') ?></label>
            <input type="text" class="widefat" id="<?php echo esc_attr($this->get_field_id('location_set_title')); ?>" name="<?php echo esc_attr($this->get_field_name('location_set_title')); ?>" value="<?php if (isset($instance['location_set_title'])) echo esc_attr($instance['location_set_title']); else echo __('Current location is %s', 'wc_catalog_restrictions'); ?>" /></p>
...
function woocommerce_catalog_restrictions_sidebar_location_picker_process() {
    ...
    if (isset($_POST['woocommerce_catalog_restrictions_location_picker']) && $_POST['woocommerce_catalog_restrictions_location_picker'] == 'sidebar') {
        // Get redirect URL
        $redirect_to = apply_filters('woocommerce_catalog_restrictions_choose_location_widget_redirect', get_permalink(woocommerce_get_page_id('shop')));
        $_SESSION['wc_location'] = filter_input(INPUT_POST, 'location', FILTER_SANITIZE_STRIPPED);
